...
const UID_EVENT = "api::event.event";
exports.default = strapi_1.factories.createCoreController(UID_EVENT, ({ strapi }) => ({
    // ------------------------------------------------
    ...
    async getYears(ctx) {
        try {
            ...
            const years = Array.from(new Set(events
                .map(ev => ev.date)
                .filter(Boolean)
                .map(date => new Date(date).getFullYear()))).sort((a, b) => a - b);
            ctx.body = { years };
        }
        catch (err) {
    ...
    async getMonths(ctx) {
        try {
            ...
            const locale = ctx.query.locale || "en";
            if (!year)
                return ctx.badRequest("Missing year");
            ...
            const months = Array.from(new Set(events
                .map(ev => ev.date)
                .filter(Boolean)
                .map(d => new Date(d).getUTCMonth() + 1))).sort((a, b) => a - b);
            ctx.body = { months };
        }
        catch (err) {
    ...
    async upcomingYears(ctx) {
        try {
            ...
            const years = Array.from(new Set(events
                .map(e => e.date)
                .filter(Boolean)
                .map(d => new Date(d).getFullYear()))).sort((a, b) => a - b);
            ctx.body = { years };
        }
        catch (err) {
